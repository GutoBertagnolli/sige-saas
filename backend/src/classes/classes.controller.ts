...
export class ClassesController {
  ...
  async create(@Body() body: any, @Headers('authorization') authorization: string | undefined, @Req() request: any) {
    const actor = await this.audit.getActor(authorization);
    assertCanManageSchools(actor, [body.schoolId], 'Voce so pode cadastrar turmas das escolas em que atua.');
    const result = await this.service.create(body);
    await this.audit.record({ authorization, entity: 'Turma', entityId: result?.id, action: 'CREATE', newData: result, ipAddress: getClientIp(request) });
    return result;
  }
  ...
  async update(@Param('id') id: string, @Body() body: any, @Headers('authorization') authorization: string | undefined, @Req() request: any) {
    ...
    assertCanManageSchools(
      ...
      'Voce so pode alterar turmas das escolas em que atua.',
    );
    const result = await this.service.update(id, body);
    await this.audit.record({ authorization, entity: 'Turma', entityId: id, action: 'UPDATE', oldData: body, newData: result, ipAddress: getClientIp(request) });
    return result;
  }
  ...
  async remove(@Param('id') id: string, @Headers('authorization') authorization: string | undefined, @Req() request: any) {
    const actor = await this.audit.getActor(authorization);
    const currentSchoolId = await this.service.getSchoolId(id);
    assertCanManageSchools(actor, [currentSchoolId].filter(Boolean), 'Voce so pode excluir turmas das escolas em que atua.');
    const result = await this.service.remove(id);
    await this.audit.record({ authorization, entity: 'Turma', entityId: id, action: 'DELETE', newData: result, ipAddress: getClientIp(request) });
    return result;
  }
}
